...
class ACNet(nn.Module):
    def __init__(self, n_classes):
        ...
        self.final_deconv = nn.ConvTranspose2d(self.inplanes, n_classes, kernel_size=2,
    ...
    def decoder(self, fuse0, fuse1, fuse2, fuse3, fuse4):
        ...
        if self.training:
            out2 = F.interpolate(out2, size=out.size()[2:], mode='bilinear', align_corners=True)
            out3 = F.interpolate(out3, size=out.size()[2:], mode='bilinear', align_corners=True)
            out4 = F.interpolate(out4, size=out.size()[2:], mode='bilinear', align_corners=True)
            out5 = F.interpolate(out5, size=out.size()[2:], mode='bilinear', align_corners=True)

            multi_out = [out2, out3, out4, out5, out]
            return tuple(multi_out)
        return out
    ...
    def _load_resnet_pretrained(self):
        ...
        pretrain_dict = torchvision.models.resnet50(pretrained=True).state_dict()
        model_dict = {}
